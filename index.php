...
class Redis_Page_Cache {
	...
	private static $ignore_request_keys = array(
		'utm_source',
		'utm_medium',
		'utm_term',
		'utm_content',
	...
	public static function cache_init() {
		...
		if ( is_array( $cache ) && ! empty( $cache ) ) {
			...
			if ( $expired && $serve_cache ) {
				...
				if ( ! $lock ) {
					$lock = $redis->set( sprintf( 'pjc-%s-lock', self::$request_hash ), true, array(
						'nx',
						'ex' => 30
					) );
					if ( $lock ) {
...
//		self::clear_cache_by_flag( 'product' );
	...
	public static function _add_action_compat() {
		...
		$GLOBALS['wp_filter']['transition_post_status'][10]['pj-page-cache'] = array(
			'function'      => array( __CLASS__, 'transition_post_status' ),
			'accepted_args' => 3
		);
		$GLOBALS['wp_filter']['template_redirect'][100]['pj-page-cache']     = array(
			'function'      => array( __CLASS__, 'template_redirect' ),
			'accepted_args' => 1
		);
	}
}
